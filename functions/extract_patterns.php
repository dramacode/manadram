...
function extract_patterns($xp, $length, $confidents, $group) {
    ...
    //prend le xp d'un fichier, la longueur des motifs �� extraire (0 pour extraire un seul motif global), les options confidents et group
    ...
    while ($k < $configurations->length) {
        $current_configuration = $first_configuration = $configurations->item($k); //conf de d�part du motif
        $conf_id = $current_configuration->getAttribute("xml:id");
    //print_r($conf_id);
        $scene_id = $xp->evaluate("./ancestor::*[@type='scene']", $first_configuration);
        $scene_id = $scene_id->item(0)->getAttribute("xml:id"); //scene_id (renvoie toujours � un id existant) pourra diff�rer de $configuration_id (-1 pour les d�buts d'acte)

        $first_act = $xp->evaluate("./ancestor::*[@type='act']", $first_configuration); // suppose que toutes les sc��nes soient dans un acte
        ...
        */
        ...
        $pattern = $group ? flatgroup($pattern) : $pattern; //� partir de l�, je perds les identifiants de personnages dans $pattern : [n][conf]value

        $patterns[$scene_id]["int_bin"] = flatbin($pattern);
        $patterns[$scene_id]["int_dec"] = bindec(flatbin($pattern));
        $pattern = flatmulti($pattern);
        $pattern = multiflip($pattern); //� partir de l�, l'array a la structure [conf][n]value
        ...
        $patterns[$scene_id]["trim_code"] = preg_replace("|/?(.+?)/?|", "$1", $patterns[$scene_id]["str_code"]);
        //id_pattern : ne pas grouper ; inutile de remove equal successive conf puisqu'on a d�j� continue si c'est le cas ;
        ...
        $patterns[$scene_id]["conf_id"] = $conf_id;//�videmment il faudrait mettre $conf_id comme clef... Cr�er un conf_n = scene_n+a/b si plusieurs conf
        $patterns[$scene_id]["l"] = $length;

        if ($all) {
            break; //on n'extrait pas les motifs suivants, uniquement un motif global � partir de la premi�re sc�ne
