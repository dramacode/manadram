              ...
              <xsl:for-each select="//tei:body//tei:*[@type = 'act']">
                ...
                <xsl:choose>
                  <xsl:when test="not(tei:*[@type = 'scene'])">
                    <xsl:variable name="n" select="count(.//tei:listPerson[@type = 'configuration'])"/>
                    <td class="scene" colspan="{$n}" id="{@xml:id}"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:for-each select="//tei:*[@xml:id = $actId]//tei:*[@type = 'scene']">
                      ...
                      <td class="scene {.//tei:listPerson[@type='configuration'][1]/@xml:id}" colspan="{$n}" id="{@xml:id}">
                        <a class="tooltip" href="http://www.theatre-classique.fr/pages/programmes/edition.php?t=../documents/{translate($basename, $lowercase, $uppercase)}.xml#A{$actN}.S{$actN}{@n}" target="_blank" title="Voir le texte">
                          <xsl:value-of select="@n"/>
                        </a>
                      </td>
                    </xsl:for-each>
        ...
        <tbody>
          <tr id="confList" style="display:none;">
            ...
            <xsl:for-each select="//tei:listPerson">
              ...
              <td id="{@xml:id}">
                <xsl:attribute name="class">configuration <xsl:value-of select="$confId"/> <xsl:if test="
                    (count(//tei:listPerson[following::tei:listPerson[@xml:id = $confId]][ancestor::tei:*[@xml:id = $actId]]) = 0)
                    or (not(.//tei:person/@corresp = //tei:listPerson[@xml:id = $previousConfId]//tei:person/@corresp)) or (@subtype = 'break')"> break</xsl:if> </xsl:attribute>
              </td>
            </xsl:for-each>
          </tr>
          ...
          <xsl:for-each select="//tei:person[not(preceding::tei:person/@corresp = ./@corresp)]">
            ...
            <tr id="{$roleId}">
              ...
              <xsl:for-each select="//tei:listPerson[@type = 'configuration']">
                ...
                <xsl:variable name="break"><xsl:if test="
                  (count(//tei:listPerson[following::tei:listPerson[@xml:id = $confId]][ancestor::tei:*[@xml:id = $actId]]) = 0)
                ...
                <xsl:choose>
                          ...
                          <xsl:when test="
                            ...
                            <xsl:choose>
                              <xsl:when test=".//tei:person[@corresp = concat('#', $roleId)][@role = 'offstage']">
                                <td class="configuration offstage {$confId} {$break}" id="{$roleId}{$confId}">1</td>
                              </xsl:when>
                              <xsl:when test=".//tei:person[@corresp = concat('#', $roleId)][@role = 'aside']">
                                <td class="configuration aside speaking {$confId}  {$break}" id="{$roleId}{$confId}">1</td>
                              </xsl:when>
                              <xsl:otherwise>
                                <td class="configuration speaking {$confId}  {$break}" id="{$roleId}{$confId}">1</td>
                              </xsl:otherwise>
                            </xsl:choose>
                          </xsl:when>
                          <xsl:otherwise>
                            <xsl:choose>
                              <xsl:when test=".//tei:person[@corresp = concat('#', $roleId)][@role = 'unconscious' or @role = 'dead']">
                                <td class="configuration dead {$confId} {$break}" id="{$roleId}{$confId}">†</td>
                              </xsl:when>
                              <xsl:when test=".//tei:person[@corresp = concat('#', $roleId)][@role = 'hidden']">
                                <td class="configuration hidden {$confId} {$break}" id="{$roleId}{$confId}">c</td>
                              </xsl:when>
                              <xsl:when test=".//tei:person[@corresp = concat('#', $roleId)][@role = 'aside']">
                                <td class="configuration aside mute {$confId} {$break}" id="{$roleId}{$confId}">1</td>
                              </xsl:when>
                              <xsl:otherwise>
                                <td class="configuration mute {$confId} {$break}" id="{$roleId}{$confId}">2</td>
                              </xsl:otherwise>
                            </xsl:choose>
                          </xsl:otherwise>
                        </xsl:choose>
                  ...
                  <xsl:otherwise>
                    <td class="configuration absent {$confId} {$break}" id="{$roleId}{$confId} {$break}">0</td>
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:for-each>
            </tr>
          </xsl:for-each>
          <xsl:for-each select="//tei:person[@n]">
            <!--[not(@corresp = preceding::tei:person/@corresp)]-->
            <xsl:variable name="roleId">
            ...
            <tr id="{$roleId}">
              ...
              <xsl:for-each select="//tei:listPerson[@type = 'configuration']">
                ...
                <xsl:choose>
                  ...
                  <xsl:otherwise>
                    <td class="configuration absent {$confId} {$break}" id="{$roleId}{$confId}">0</td>
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:for-each>
              <!--<td>
